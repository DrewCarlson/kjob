...
import kjob.core.internal.DefaultJobScheduler
...
import kjob.core.internal.scheduler.sj
...
class BaseKJobSpec : ShouldSpec() {
    ...
    object TestModule :
        ExtensionModule<TestEx, TestEx.Configuration, BaseKJob<BaseKJob.Configuration>, BaseKJob.Configuration> {
    ...
    init {
        should("execute a new job as expected") {
            ...
            coEvery {
                jobRepoMock.findNext(
                    emptySet(),
                    setOf(JobStatus.SCHEDULED, JobStatus.RUNNING, JobStatus.ERROR),
                    50
                )
            } returns emptyFlow()
            coEvery { jobRepoMock.save(settings, null) } returns sj
            coEvery { jobRepoMock.update(sj.id, null, testee.id, JobStatus.SCHEDULED, null, 0) } returns true
            coEvery { jobRepoMock.update(sj.id, testee.id, testee.id, JobStatus.RUNNING, null, 0) } returns true
        ...
        should("execute a delayed job as expected") {
            ...
            coEvery { jobRepoMock.update(sj1.id, null, testee.id, JobStatus.SCHEDULED, null, 0) } returns true
            coEvery { jobRepoMock.update(sj2.id, null, testee.id, JobStatus.SCHEDULED, null, 0) } returns true
            coEvery { jobRepoMock.update(sj1.id, testee.id, testee.id, JobStatus.RUNNING, null, 0) } returns true
            coEvery { jobRepoMock.update(sj2.id, testee.id, testee.id, JobStatus.RUNNING, null, 0) } returns true
            coEvery { jobRepoMock.findNextOne(setOf("test-job"), setOf(JobStatus.CREATED)) } returns sj1 andThen sj2 andThen null
            ...
            coEvery { jobRepoMock.update(sj1.id, testee.id, null, JobStatus.COMPLETE, null, 1) } returns true
            coEvery { jobRepoMock.update(sj2.id, testee.id, null, JobStatus.COMPLETE, null, 1) } returns true
            testee.start()
            testee.register(TestJob) {
                execute {
                    setInitialMax(4)
            ...
            coVerify(timeout = 200) { jobRepoMock.update(sj1.id, testee.id, null, JobStatus.COMPLETE, null, 1) }
            coVerify(timeout = 200) { jobRepoMock.update(sj2.id, testee.id, null, JobStatus.COMPLETE, null, 1) }
        }
        ...
        should("fail to schedule job if the same id has already been used") {
            ...
            coEvery {
                jobRepoMock.findNext(
                    emptySet(),
                    setOf(JobStatus.RUNNING, JobStatus.ERROR),
                    50
                )
            } returns emptyFlow()
            coEvery { jobRepoMock.findNextOne(setOf("test-job"), setOf(JobStatus.CREATED)) } returns sj andThen null
