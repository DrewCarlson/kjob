...
) : BaseExtension(MyGenericExtension) {
...
object MyGenericModule :
    ...
    override fun create(
        configure: MyGenericEx.Configuration.() -> Unit,
        kjobConfig: BaseKJob.Configuration
    ): (BaseKJob<BaseKJob.Configuration>) -> MyGenericEx {
...
class MyInMemEx(
    private val config: Configuration,
    private val inmemConfig: InMemKJob.Configuration,
    private val kjob: InMemKJob
) : BaseExtension(MyInMemExtension) {
    class Configuration : BaseExtension.Configuration() {
...
object MyInMemModule : ExtensionModule<MyInMemEx, MyInMemEx.Configuration, InMemKJob, InMemKJob.Configuration> {
    ...
    override fun create(
        configure: MyInMemEx.Configuration.() -> Unit,
        kjobConfig: InMemKJob.Configuration
    ): (InMemKJob) -> MyInMemEx {
...
class MyMongoEx(
    private val config: Configuration,
    private val mongoConfig: MongoKJob.Configuration,
    private val kjob: MongoKJob
) : BaseExtension(MyMongoExtension) {
    class Configuration : BaseExtension.Configuration() {
...
object MyMongoModule : ExtensionModule<MyMongoEx, MyMongoEx.Configuration, MongoKJob, MongoKJob.Configuration> {
    ...
    override fun create(
        configure: MyMongoEx.Configuration.() -> Unit,
        kjobConfig: MongoKJob.Configuration
    ): (MongoKJob) -> MyMongoEx {
